...
var OpenloadDecoder = {
    decode: function(html) {
        ...
        */
        ...
        try {
            var spanMatches = getMatches(html, /<span[^>]+id="[^"]+"[^>]*>([0-9A-Za-z]+)<\/span>/gi, 1);

            for (var spanIdx = 0; spanIdx < spanMatches.length; spanIdx++) {
                try {
                    var output = "";

                    var encoded = spanMatches[spanIdx]; //uv1

                    var uv2vMatches = getMatches(html, /\^([a-f0-9x]+)/g, 1);
                    var uv2v = parseInt(uv2vMatches[0], 16);

                    var uv3Matches = getMatches(html, /\\x31\\x31'\s*:\s*var\s*_[0-1]x[0-9a-f]{6}\s*=\s*([0-9a-fx]+)/g, 1);
                    var uv3 = parseInt(uv3Matches[0], 16);

                    Log.d("encoded = " + encoded);
                    Log.d("uv2v = " + uv2v);
                    Log.d("uv3 = " + uv3);

                    var d = encoded.substr(0, 72);
                    var f = []; //long list
                    ...
                    while (a < encoded.length) {
                        ...
                        for (var i = 0; i < 4; i++) {
                            ...
                            var t = String.fromCharCode(parseInt(r));
                    ...
                    if (output.length >= 5)
                        results.push("https://openload.co/stream/" + output + "?mime=true");
                } catch (err1) {
...
function aadecode(text) {
    var evalPreamble = "(ï¾ŸÐ”ï¾Ÿ) ['_'] ( (ï¾ŸÐ”ï¾Ÿ) ['_'] (";
    var decodePreamble = "( (ï¾ŸÐ”ï¾Ÿ) ['_'] (";
    var evalPostamble = ") (ï¾ŸÎ˜ï¾Ÿ)) ('_');";
    var decodePostamble = ") ());";
